...
let nodes_active_on_some_chain info =
  List.(
    filter ~f:(fun n ->
        exists ~f:(fun c -> mem (active info n) c ~equal:Chain.equal)
        @@ chains info)
    @@ nodes info)
...
let nodes_inactive_on_some_chain info =
  List.(
    filter ~f:(fun n ->
        exists ~f:(fun c -> mem (inactive info c) n ~equal:Id.equal)
...
let has_block_on_chain info node chain =
  List.exists ~f:(has_block_on_branch info node chain)
  @@ branches info node chain

let has_block info node =
  List.exists ~f:(has_block_on_chain info node) @@ chains info

let nodes_with_blocks info = List.filter ~f:(has_block info) @@ nodes info

let chains_with_blocks info node =
  List.filter ~f:(has_block_on_chain info node) @@ chains info

let branches_with_blocks info node chain =
  List.filter ~f:(has_block_on_branch info node chain)
  @@ branches info node chain
...
let block_at_height_on info node chain branch height =
  try
    let open List in
    hd
    @@ filter ~f:(fun (b : Block.t) -> b.header.height = height)
...
let chains_with_branches info node =
  List.filter ~f:(has_branch_on_chain info node) @@ chains info

let has_branches info node =
  List.exists ~f:(has_branch_on_chain info node) @@ chains info

let nodes_with_branches info = List.filter ~f:(has_branches info) @@ nodes info
...
let has_expect info node =
  List.exists ~f:(has_expect_on_chain info node) @@ chains info

let nodes_with_expect info = List.filter ~f:(has_expect info) @@ nodes info

let chains_with_expect info node =
  List.filter ~f:(has_expect_on_chain info node) @@ chains info
...
let has_headers info node =
  List.exists ~f:(has_headers_on_chain info node) @@ chains info

let nodes_with_headers info = List.filter ~f:(has_headers info) @@ nodes info

let chains_with_headers info node =
  List.filter ~f:(has_headers_on_chain info node) @@ chains info
...
let has_height_on_chain info node chain =
  List.exists ~f:(has_height_on_branch info node chain)
  @@ branches info node chain

let has_height info node =
  List.exists ~f:(has_headers_on_chain info node) @@ chains info

let nodes_with_height info = List.filter ~f:(has_height info) @@ nodes info

let chains_with_height info node =
  List.filter ~f:(has_height_on_chain info node) @@ chains info
...
let has_msgs info node =
  List.exists ~f:(has_msgs_on_chain info node) @@ chains info

let nodes_with_msgs info = List.filter ~f:(has_msgs info) @@ nodes info

let chains_with_msgs info node =
  List.filter ~f:(has_msgs_on_chain info node) @@ chains info

let peek info node chain = Queue.peek_exn @@ messages info node chain

let peek_opt info node chain = Queue.peek @@ messages info node chain
